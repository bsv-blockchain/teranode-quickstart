...
If you need the full historical block data (indexers, explorers, chain analysis), enable the optional **blockpersister** service by adding `archival` to `COMPOSE_PROFILES` in `.env` (e.g. `COMPOSE_PROFILES=archival` or `COMPOSE_PROFILES=full,archival`). It writes raw block data to disk and runs in parallel with the pruner — pruning behaviour is unchanged, you just gain a durable archive.

Archival mode costs significant disk space (grows with chain size — budget multiple TB on mainnet). Most operators should leave `archival` out of `COMPOSE_PROFILES` and rely on the default 288-block pruning window.
...
- `clientName` — identity shown in explorer
- `listen_mode` — `listen_only` or empty (full mode)
- `rpc_user`, `rpc_pass` — generated by `setup.sh`
- `asset_httpPublicAddress`, `p2p_advertise_addresses` — only needed for full mode
- `minminingtxfee`, `blockmaxsize`, `excessiveblocksize` — per-network consensus knobs
- `logLevel`, `p2p_dht_mode`, `p2p_health_check_enabled`

`setup.sh` writes sensible defaults; edit any of these directly if you need to. There is no rename or translation layer — the key in `.env` is the same key Teranode reads. `SETTINGS_CONTEXT=docker.m` picks up upstream defaults from the image's built-in `settings.conf`; env vars override the keys you care about.
...
```
teranode-quickstart/
├── setup.sh, start.sh, stop.sh, cli.sh, rpc.sh, logs.sh, status.sh, clean.sh, update.sh, seed.sh
